...
- google-spreadsheet
- google-auth-library
- Built-in auth session module (Bearer token + role hierarchy)
...
```text
1CHXlpCmanz8BxvPg91898g_aFoMKKIWx-16n0SyM3wY
```
...
Mọi endpoint nghiệp vụ đều dùng `Authorization: Bearer <token>`.
...
- Head quản lý toàn bộ user.
- CSS Manager quản lý Team Leader + Staff trong nhánh của mình.
- CSS Team Leader quản lý Staff trong nhánh của mình.
- Staff chỉ xem dashboard theo scope được cấp.
...
Trả về payload dashboard từ SQLite và **bắt buộc đăng nhập**.
...
## 13. Bootstrap tài khoản mặc định

Nếu database chưa có user nào, hệ thống sẽ tự tạo 1 tài khoản Head mặc định khi khởi động server:
...
```bash
BOOTSTRAP_HEAD_EMAIL=your-head@company.com
BOOTSTRAP_HEAD_PASSWORD='StrongPasswordHere'
```

Khuyến nghị: đăng nhập lần đầu và đổi/cấp lại user thật ngay sau khi bootstrap.
